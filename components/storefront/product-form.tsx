...
export function ProductForm({
  ...
  // Sin fallback a variants[0]: si la combinación no existe hay que decirlo, no
  // cobrar la primera variante del producto a espaldas del cliente.
  ...
  /**
   * Un valor se puede elegir si existe alguna variante que lo combine con lo ya
   * seleccionado en las demás opciones. Evita callejones como pedir un pack de
   * 10 en un espesor que solo se vende por pieza.
   */
  const esCombinable = useMemo(() => {
    const cache = new Map<string, boolean>();
    return (optionName: string, value: string) => {
      const clave = `${optionName}::${value}`;
      const previo = cache.get(clave);
      if (previo !== undefined) return previo;
      const existe = variants.some((variant) =>
  ...
  const price = selectedVariant ? Number(selectedVariant.price.amount) : 0;
    ...
    <div>
      <div className="flex items-center gap-3">
        <span className="font-display text-3xl text-gold">
          {selectedVariant ? formatMXN(price) : "No disponible"}
        </span>
        {hasDiscount && (
          <>
          ...
          <div key={option.name} className="mt-6">
            ...
            <div className="flex flex-wrap gap-2">
              {option.values.map((value) => {
                ...
                const disponible = esCombinable(option.name, value);
                return (
                  <button
                    key={value}
                    onClick={() => onSelect(option.name, value)}
                    disabled={!disponible && !isSelected}
                    title={disponible ? undefined : "No disponible con lo que ya elegiste"}
                    className={cn(
                      ...
                      isSelected
                        ...
                        : disponible
                          ...
                          : "border-ink-border/40 text-graystone-500 line-through cursor-not-allowed"
                    )}
                  >
                    {value}
                  </button>
