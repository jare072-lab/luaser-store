...
- **Brief critic** judges against the stated goal only. Does it do the thing? Ignore aesthetics. Run on Sonnet.
- **System critic** judges against `design-system.md` only. Objective adherence. Run on Haiku: this is mechanical checking, not taste.
- **Craft critic** judges against `bar.md` and rendered output only. Put ours next to the reference blind with labels stripped, say which is better, name the single biggest gap. Run on the strongest model available and never downgrade it. A cheap craft critic approves everything, which kills the loop.
